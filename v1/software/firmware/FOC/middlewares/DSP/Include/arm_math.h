...
/**
   ...
   * The library installer contains a project file to rebuild libraries on MDK
   toolchain in the <code>CMSIS\\DSP\\Projects\\ARM</code> folder.
   ...
   * - ARM_MATH_LOOPUNROLL:
   *
   * Define macro ARM_MATH_LOOPUNROLL to enable manual loop unrolling in DSP
   functions
   *
   * - ARM_MATH_NEON:
   *
   * Define macro ARM_MATH_NEON to enable Neon versions of the DSP functions.
   * It is not enabled by default when Neon is available because performances
   are
   * dependent on the compiler and target architecture.
   *
   * - ARM_MATH_NEON_EXPERIMENTAL:
   *
   * Define macro ARM_MATH_NEON_EXPERIMENTAL to enable experimental Neon
   versions of
   * of some DSP functions. Experimental Neon versions currently do not have
   better
   ...
   * |\b CMSIS\\DSP\\Examples          | Example projects demonstrating the
 ...
 * Refer to \ref arm_mat_init_f32(), \ref arm_mat_init_q31() and \ref
 * arm_mat_init_q15() for floating-point, Q31 and Q15 types,  respectively.
 *
 * \par
 ...
 */
__STATIC_FORCEINLINE uint32_t arm_recip_q31(q31_t in, q31_t *dst,
 ...
 */
__STATIC_FORCEINLINE uint32_t arm_recip_q15(q15_t in, q15_t *dst,
...
typedef struct {
    ...
    const q7_t *pCoeffs; /**< points to the coefficient array. The array is of
                            length numTaps.*/
} arm_fir_instance_q7;
...
typedef struct {
    ...
    q15_t *pState;    /**< points to the state variable array. The array is of
                         length numTaps+blockSize-1. */
    const q15_t *pCoeffs; /**< points to the coefficient array. The array is of
                             length numTaps.*/
...
typedef struct {
    ...
    q31_t *pState;    /**< points to the state variable array. The array is of
                         length numTaps+blockSize-1. */
    const q31_t *pCoeffs; /**< points to the coefficient array. The array is of
                             length numTaps. */
...
typedef struct {
    ...
    float32_t *pState; /**< points to the state variable array. The array is of
                          length numTaps+blockSize-1. */
    const float32_t *pCoeffs; /**< points to the coefficient array. The array is
                                 of length numTaps. */
 ...
 * <code>ARM_MATH_ARGUMENT_ERROR</code> if <code>numTaps</code> is not a
 * supported value.
 */
...
typedef struct {
    ...
    const q15_t *pCoeffs; /**< Points to the array of coefficients.  The array
...
typedef struct {
    ...
    q31_t *pState; /**< Points to the array of state coefficients.  The array is
                      of length 4*numStages. */
    const q31_t *pCoeffs; /**< Points to the array of coefficients.  The array
                             is of length 5*numStages. */
...
typedef struct {
    ...
    const float32_t *pCoeffs; /**< Points to the array of coefficients.  The
                                 array is of length 5*numStages. */
...
typedef struct {
    ...
    uint8_t
        bitReverseFlag;    /**< flag that enables (bitReverseFlag=1) or disables
...
typedef struct {
    ...
    uint32_t twidCoefRModifier; /**< twiddle coefficient modifier that supports
                                   ...
                                   factor table. */
    const float32_t
        *pTwiddleAReal; /**< points to the real twiddle factor table. */
    const float32_t
        *pTwiddleBReal; /**< points to the imag twiddle factor table. */
    arm_cfft_radix4_instance_f32
        *pCfft; /**< points to the complex FFT instance. */
...
typedef struct {
    uint8_t numStages;  /**< number of 2nd order stages in the filter.  Overall
                           order is 2*numStages. */
    float64_t *pState;  /**< points to the array of state coefficients.  The
                           array is of length 2*numStages. */
    float64_t *pCoeffs; /**< points to the array of coefficients.  The array is
...
/**
  ...
  intermediate multiplication results but provides only a single guard bit.
         Thus, if the accumulator result overflows it wraps around rather than
  clip. In order to avoid overflows completely the input signal must be scaled
  down by 2 bits as there are four additions. After all multiply-accumulates are
...
__STATIC_FORCEINLINE q15_t arm_pid_q15(arm_pid_instance_q15 *S, q15_t in) {
    ...
    acc = (q63_t)__SMLALD((uint32_t)S->A1, (uint32_t)read_q15x2(S->state),
                          (uint64_t)acc);
#else
    /* acc = A0 * x[n]  */
    acc = ((q31_t)S->A0) * in;
 ...
 * figure below
 * \image html clarke.gif Stator current space vector and its components in
 * (a,b). and <code>Ia + Ib + Ic = 0</code>, in this condition
 ...
 * functions for Q31 and floating-point data types.
 * \par Algorithm
 * \image html clarkeFormula.gif
 * where <code>Ia</code> and <code>Ib</code> are the instantaneous stator phases
 * and <code>pIalpha</code> and <code>pIbeta</code> are the two coordinates of
 ...
 * @return        none
 */
__STATIC_FORCEINLINE void arm_clarke_f32(float32_t Ia, float32_t Ib,
                                         float32_t *pIalpha,
                                         float32_t *pIbeta) {
 ...
 */
__STATIC_FORCEINLINE void arm_clarke_q31(q31_t Ia, q31_t Ib, q31_t *pIalpha,
                                         q31_t *pIbeta) {
    q31_t product1,
 ...
 * where <code>pIa</code> and <code>pIb</code> are the instantaneous stator
 * phases and <code>Ialpha</code> and <code>Ibeta</code> are the two coordinates
 ...
 * @return     none
 */
__STATIC_FORCEINLINE void arm_inv_clarke_f32(float32_t Ialpha, float32_t Ibeta,
 ...
 */
__STATIC_FORCEINLINE void arm_inv_clarke_q31(q31_t Ialpha, q31_t Ibeta,
                                             q31_t *pIa, q31_t *pIb) {
    q31_t product1,
        product2; /* Temporary variables used to store intermediate results */
 ...
 * where <code>Ialpha</code> and <code>Ibeta</code> are the stator vector
 * components, <code>pId</code> and <code>pIq</code> are rotor vector components
 * and <code>cosVal</code> and <code>sinVal</code> are the cosine and sine
 * values of theta (rotor flux position).
 * \par Fixed-Point Behavior
 ...
 *
 */
__STATIC_FORCEINLINE void arm_park_f32(float32_t Ialpha, float32_t Ibeta,
 ...
 */
__STATIC_FORCEINLINE void arm_park_q31(q31_t Ialpha, q31_t Ibeta, q31_t *pId,
 ...
 * where <code>pIalpha</code> and <code>pIbeta</code> are the stator vector
 * components, <code>Id</code> and <code>Iq</code> are rotor vector components
 * and <code>cosVal</code> and <code>sinVal</code> are the cosine and sine
 * values of theta (rotor flux position).
 * \par Fixed-Point Behavior
 * Care must be taken when using the Q31 version of the Park transform.
 ...
 * @return     none
 */
__STATIC_FORCEINLINE void arm_inv_park_f32(float32_t Id, float32_t Iq,
                                           float32_t *pIalpha,
                                           float32_t *pIbeta, float32_t sinVal,
                                           float32_t cosVal) {
...
/**
  ...
  addition, hence there is no risk of overflow.
 */
__STATIC_FORCEINLINE void arm_inv_park_q31(q31_t Id, q31_t Iq, q31_t *pIalpha,
 ...
 *
 */
__STATIC_FORCEINLINE float32_t
 ...
 *
 */
__STATIC_FORCEINLINE q31_t arm_linear_interp_q31(q31_t *pYData, q31_t x,
                                                 uint32_t nValues) {
    q31_t y;       /* output */
 ...
 *
 */
__STATIC_FORCEINLINE q15_t arm_linear_interp_q15(q15_t *pYData, q31_t x,
                                                 uint32_t nValues) {
    q63_t y;       /* output */
 ...
 * maximum of table size 2^12.
 */
__STATIC_FORCEINLINE q7_t arm_linear_interp_q7(q7_t *pYData, q31_t x,
                                               uint32_t nValues) {
    q31_t y;        /* output */
    ...
    if (index >= (nValues - 1)) {
        ...
        /* Read two nearest output values from the index and are in 1.7(q7)
         * format */
        y0 = pYData[index];
        y1 = pYData[index + 1];
 ...
 */
__STATIC_FORCEINLINE void arm_circularWrite_q7(q7_t *circBuffer, int32_t L,
                                               uint16_t *writeOffset,
                                               int32_t bufferInc,
                                               const q7_t *src, int32_t srcInc,
                                               uint32_t blockSize) {
    uint32_t i = 0U;
    int32_t wOffset;
 ...
 * buffer.
 */
void arm_min_q7(const q7_t *pSrc, uint32_t blockSize, q7_t *result,
                uint32_t *index);
